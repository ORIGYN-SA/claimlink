...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const getPageSubtitle = (pathname: string): string | undefined => {
    if (pathname.startsWith("/mint_nft"))
      return "You can manage your organization's collections that hosts your certificates";
    if (pathname.startsWith("/account/new"))
      return "Create and manage your organization's user roles and permissions";
    if (pathname.startsWith("/account/edit_company"))
      return "Create and manage your organization's user roles and permissions";
    if (pathname.startsWith("/account/edit_user"))
      return "Create and manage your organization's user roles and permissions";
    if (pathname.startsWith("/templates/new"))
      return "Create a new certificate template in 3 simple steps";
    return undefined;
  };
  ...
  return (
    <div className="min-h-screen bg-[#222526] relative">
      ...
      <div className="absolute inset-0 backdrop-blur-[100px] backdrop-filter bg-[rgba(19,19,19,0.4)]" />
      <div className="relative flex">
        ...
        <main className="flex-1 px-3 py-3 sm:px-4 sm:py-4 lg:px-6 lg:py-6 min-w-0">
          <div className="bg-[#fcfafa] rounded-[12px] sm:rounded-[16px] lg:rounded-[20px] p-4 sm:p-5 lg:p-[24px] w-full max-w-none min-h-[calc(100dvh-24px)] lg:min-h-[calc(100dvh-48px)] flex flex-col">
            <HeaderBar
              ...
              backTo={backTo}
            />
            <div className="mt-4 sm:mt-5 lg:mt-6 flex-1 flex flex-col">
              {children}
            </div>
          </div>
        </main>
