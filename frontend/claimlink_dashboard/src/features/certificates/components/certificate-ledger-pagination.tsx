...
export function CertificateLedgerPagination({
  ...
  return (
    <div className="bg-white flex items-center justify-between px-10 py-4 rounded-bl-[25px] rounded-br-[25px]">
      {/* Lines per page */}
      <div className="flex gap-2.5 items-center">
        <p className="text-[#69737c] text-[13px] font-normal leading-normal">
          Lines per page
        </p>
        <button
          className="bg-white border border-[#e1e1e1] flex gap-1.5 items-center px-2.5 py-1.5 rounded-full hover:bg-[#f9f8f4] transition-colors"
          onClick={() => {
          ...
          {renderPageNumbers().map((page, index) => {
            if (page === "...") {
              return (
                <span
                  key={`ellipsis-${index}`}
                  className="text-[#69737c] text-[13px] font-normal"
                >
                  ...
                </span>
              );
            ...
            return (
              <button
                ...
                className={`relative min-w-[28px] h-7 flex items-center justify-center rounded-[11px] transition-colors ${
                  isActive
