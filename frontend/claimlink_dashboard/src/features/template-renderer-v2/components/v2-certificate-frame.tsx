/**
 * V2 Certificate Frame
 *
 * Visual wrapper for certificate view. Reads config from V2CertificateFrameConfig.
 * Supports both standard gradient and custom background modes.
 * Same visual output as v1 CertificateFrame but driven by template document config.
 */
...
import type { V2CertificateFrameConfig } from '../types';
...
interface V2CertificateFrameProps {
  config?: V2CertificateFrameConfig;
  content: V2LayoutNode[];
}

export function V2CertificateFrame({ config, content }: V2CertificateFrameProps) {
  const { getImageUrl, tokenId } = useV2Context();

  // Resolve stamp and logo from config field references
  const companyLogo = config?.companyLogoField ? getImageUrl(config.companyLogoField) : null;
  const stampUrl = config?.stampField ? getImageUrl(config.stampField) : null;
  const stampSrc = stampUrl || stampStandard;
  const displayTokenId = (config?.showTokenId !== false && tokenId) ? tokenId : '';
  const showBranding = config?.showOrigynBranding !== false;

  const hasCustomBackground = config?.background?.type === 'custom' && config.background.dataUri;
  const isVideoBackground = hasCustomBackground && config?.background?.mediaType === 'video';
  const variant = hasCustomBackground ? 'custom-certificate' as const : 'certificate' as const;

  // Custom background layout
  if (hasCustomBackground) {
    return (
      <div className="rounded-bl-[24px] rounded-br-[24px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] w-full">
        <div className="relative w-[950px] h-[1350px] mx-auto">
          ...
          <div className="relative p-[64px] flex flex-col items-center h-full">
            ...
            <div
              ...
              style={{ background: 'rgba(2, 2, 2, 0.2)', backdropFilter: 'blur(8px)' }}
            >
              {/* Stamp */}
              <div className="absolute -top-[35px] sm:-top-[55px] left-0 right-0 z-20 flex justify-center">
                <img alt="Blockchain Certified" src={stampSrc} className="w-[70px] h-[70px] sm:w-[110px] sm:h-[110px]" />
              </div>

              {/* Company Logo */}
              <div className="flex justify-center w-full">
                {companyLogo ? (
                  <CanisterImage alt="Company Logo" src={companyLogo} className="h-[32px] sm:h-[48px] object-contain brightness-0 invert" />
                ) : (
              ...
              <div className="flex flex-col gap-8 items-center justify-center w-full">
                <V2ContentRenderer content={content} variant={variant} />
              </div>

              {/* Footer */}
              {showBranding && (
                <div className="flex flex-col gap-2 sm:gap-3 items-center">
                  <img alt="ORIGYN" src={logoTransparent} className="h-[40px] w-[42px] sm:h-[56px] sm:w-[58px] object-contain brightness-0 invert" />
  ...
  return (
    <div className="bg-[#fcfafa] rounded-bl-[24px] rounded-br-[24px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] w-full">
          ...
          <div className="bg-[#fcfafa] rounded-2xl relative h-full">
            ...
            <div className="absolute bottom-0 left-0 right-0 h-[300px] sm:h-[400px] overflow-hidden opacity-60">
              <div
                ...
                style={{
                  background: 'radial-gradient(ellipse at center bottom, rgba(168,237,234,0.4) 0%, rgba(254,214,227,0.3) 30%, rgba(210,153,194,0.2) 50%, transparent 70%)',
                }}
              />
            </div>
            ...
            <div className="absolute top-0 left-0 right-0 z-10 flex justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 312 156" fill="none" className="w-[200px] h-[100px] sm:w-[312px] sm:h-[156px]">
                <path d="M156 0H0C43.0793 0 78 34.9207 78 78C78 121.079 112.921 156 156 156C199.079 156 234 121.079 234 78C234 34.9207 268.928 0 312.007 0H156.007H156Z" fill="#061937" />
              </svg>
            </div>
            ...
            <div className="relative z-10 p-[64px] flex flex-col justify-between items-center h-full">
              ...
              <div className="flex justify-between w-full">
                <div className="flex items-center pb-4 flex-1 border-b border-[rgba(105,115,124,0.2)]">
                  {companyLogo ? (
                    <CanisterImage alt="Company Logo" src={companyLogo} className="h-[28px] sm:h-[40px] object-contain" />
                  ) : (
              ...
              {showBranding && (
                <div className="flex flex-col gap-2 sm:gap-3 items-center">
                  <img alt="ORIGYN" src={logoTransparent} className="h-[50px] w-[52px] sm:h-[70px] sm:w-[72px] object-contain" />
