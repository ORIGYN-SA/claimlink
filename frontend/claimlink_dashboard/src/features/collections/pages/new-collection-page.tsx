...
export function NewCollectionPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Step 5: Upload and set logo if provided (now safe - collection is TemplateUploaded)
      if (imageFile) {
        try {
          setSubmitButtonText('Uploading logo...');
          toast.info('Uploading collection logo...');

          // Upload through ClaimLink proxy (handles validation and retries per chunk)
          const logoUrl = await CollectionsService.proxyUploadLogo(
            authenticatedAgent!,
            canisterId,
            imageFile,
            (progress) => {
