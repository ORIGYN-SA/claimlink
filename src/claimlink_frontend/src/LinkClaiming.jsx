...
const LinkClaiming = () => {
  ...
  const { backend, principal, isConnected } = useAuth();
  ...
  const handleClaim = async () => {
    if (!isConnected) {
      setShowModal(true); // Open modal if not connected
      return;
    }
  ...
  const renderNftDetails = () => {
    if (loadingData) {
      return (
        <div className="my-auto flex justify-center items-start mt-16 text-3xl text-gray-300 animate-pulse">
          <InfinitySpin visible={true} width="200" color="#564BF1" />
        </div>
      );
    }
    ...
    return (
      <motion.div
        ...
        className="bg-white px-4 py-4 mt-8 rounded-xl"
      >
        ...
        <div className="border border-gray-50 my-4 w-full"></div>
